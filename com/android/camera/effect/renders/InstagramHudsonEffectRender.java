...
public class InstagramHudsonEffectRender extends RGBTransEffectRender {
    ...
    protected InstagramHudsonEffectRender(GLCanvas gLCanvas, int i) {
        super(gLCanvas, i);
    }

    public static Render create(GLCanvas gLCanvas, int i) {
        Render curveEffectRender = new CurveEffectRender(gLCanvas, i);
        curveEffectRender.setRGBTransLutBuffer(getCurveRGBLutBuffer());
        Render instagramHudsonEffectRender = new InstagramHudsonEffectRender(gLCanvas, i);
        instagramHudsonEffectRender.setRGBTransLutBuffer(getSelfRGBLutBuffer());
        return new PipeRenderPair(gLCanvas, curveEffectRender, instagramHudsonEffectRender, false);
    }

    private static IntBuffer getCurveRGBLutBuffer() {
        if (sCurveRGBLutBuffer == null) {
            int[] iArr = new int[sCurveRLut.length];
            for (int i = 0; i < iArr.length; i++) {
                iArr[i] = (((sCurveBLut[i] << 16) | -16777216) | (sCurveGLut[i] << 8)) | sCurveRLut[i];
            }
            sCurveRGBLutBuffer = IntBuffer.wrap(iArr);
        }
