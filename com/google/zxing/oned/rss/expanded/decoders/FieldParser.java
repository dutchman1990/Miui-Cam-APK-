...
final class FieldParser {
        ...
        if (str.length() >= 2) {
            ...
            while (i < length) {
                ...
                if (objArr[0].equals(substring)) {
                    return objArr[1] != VARIABLE_LENGTH ? processFixedAI(2, ((Integer) objArr[1]).intValue(), str) : processVariableAI(2, ((Integer) objArr[2]).intValue(), str);
                } else {
                    i++;
                }
            }
            if (str.length() >= 3) {
                ...
                objArr2 = THREE_DIGIT_DATA_LENGTH;
                length = objArr2.length;
                i = 0;
                while (i < length) {
                    objArr = objArr2[i];
                    if (objArr[0].equals(substring2)) {
                        return objArr[1] != VARIABLE_LENGTH ? processFixedAI(3, ((Integer) objArr[1]).intValue(), str) : processVariableAI(3, ((Integer) objArr[2]).intValue(), str);
                    } else {
                        i++;
                    }
                }
                objArr2 = THREE_DIGIT_PLUS_DIGIT_DATA_LENGTH;
                length = objArr2.length;
                i = 0;
                while (i < length) {
                    objArr = objArr2[i];
                    if (objArr[0].equals(substring2)) {
                        return objArr[1] != VARIABLE_LENGTH ? processFixedAI(4, ((Integer) objArr[1]).intValue(), str) : processVariableAI(4, ((Integer) objArr[2]).intValue(), str);
                    } else {
                        i++;
                    }
                }
    ...
    private static String processFixedAI(int i, int i2, String str) throws NotFoundException {
        if (str.length() >= i) {
            ...
            if (str.length() >= i + i2) {
                ...
                return parseFieldsInGeneralPurpose != null ? new StringBuilder(String.valueOf(stringBuilder)).append(parseFieldsInGeneralPurpose).toString() : stringBuilder;
            } else {
                throw NotFoundException.getNotFoundInstance();
            }
        }
    ...
    private static String processVariableAI(int i, int i2, String str) throws NotFoundException {
        String substring = str.substring(0, i);
        int length = str.length() >= i + i2 ? i + i2 : str.length();
